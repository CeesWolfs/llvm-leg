...
void CeespuInstrInfo::copyPhysReg(MachineBasicBlock &MBB,
                               ...
                               bool KillSrc) const {
  if (Ceespu::GPRRegClass.contains(DestReg, SrcReg))
    BuildMI(MBB, I, DL, get(Ceespu::MOV_rr), DestReg)
...
void CeespuInstrInfo::storeRegToStackSlot(MachineBasicBlock &MBB,
  ...
  if (RC == &Ceespu::GPRRegClass)
    BuildMI(MBB, I, DL, get(Ceespu::STW))
...
void CeespuInstrInfo::loadRegFromStackSlot(MachineBasicBlock &MBB,
  ...
  if (RC == &Ceespu::GPRRegClass)
    BuildMI(MBB, I, DL, get(Ceespu::LDW), DestReg).addFrameIndex(FI).addImm(0);
  else
...
unsigned CeespuInstrInfo::InsertBranch(MachineBasicBlock &MBB,
                                    MachineBasicBlock *TBB,
                                    MachineBasicBlock *FBB,
  ...
  if (Cond.empty()) {
    ...
    BuildMI(&MBB, DL, get(Ceespu::JMP)).addMBB(TBB);
    return 1;
  }
...
unsigned CeespuInstrInfo::RemoveBranch(MachineBasicBlock &MBB) const {
  MachineBasicBlock::iterator I = MBB.end();
  unsigned Count = 0;
