...
public:
  ...
  CeespuAsmBackend(bool IsLittleEndian)
...
bool CeespuAsmBackend::writeNopData(uint64_t Count, MCObjectWriter *OW) const {
  if ((Count % 8) != 0)
    return false;
...
void CeespuAsmBackend::applyFixup(const MCFixup &Fixup, char *Data,
...
MCObjectWriter *CeespuAsmBackend::createObjectWriter(raw_pwrite_stream &OS) const {
  return createCeespuELFObjectWriter(OS, 0, IsLittleEndian);
}
}
...
MCAsmBackend *llvm::createCeespuAsmBackend(const Target &T,
  ...
  return new CeespuAsmBackend(/*IsLittleEndian=*/true);
}

MCAsmBackend *llvm::createCeespubeAsmBackend(const Target &T,
                                          const MCRegisterInfo &MRI,
                                          const Triple &TT, StringRef CPU) {
  return new CeespuAsmBackend(/*IsLittleEndian=*/false);
}
